                        ...
                        <div class="text-center">
                            <div class="row d-flex justify-content-center text-center">
                                ...
                                <p class="mt-3">{{ __('By purchasing you confirm our') }} <a
                                        href="{{ url('/') . '/terms' }}"
                                    >{{ __('Terms and Conditions') }}</a> </p>
                            </div>
                        </div>
                    </div>
                    <div class="col-sm-4 col-lg-4">
                        <div
                            class="card card-md w-full border-0 bg-[#f3f5f8] text-center text-heading-foreground group-[.theme-dark]/body:!bg-[rgba(255,255,255,0.02)]">
                            ...
                            <div class="card-body flex flex-col !p-[45px_50px_50px] text-center">
                                <div
                                    class="mb-[15px] mt-0 flex w-full items-end justify-center text-[50px] leading-none text-heading-foreground">
                                    ...
                                    @if (currencyShouldDisplayOnRight(currency()->symbol))
                                        ...
                                        @if ($plan->price !== ($newDiscountedPrice ?? $plan->price))
                                            <small class="mb-[0.3em] inline-flex text-[0.35em] font-normal"><span
                                                ...
                                                >{{ $plan->price }}</span>{{ currency()->symbol }}</small>
                                            &nbsp;
                                            {{ $newDiscountedPrice }}<small
                                                class="mb-[0.3em] inline-flex text-[0.35em] font-normal"
                                            >{{ currency()->symbol }}</small>
                                        @else
                                            {{ $plan->price }}
                                            <small
                                                class="mb-[0.3em] inline-flex text-[0.35em] font-normal">{{ currency()->symbol }}</small>
                                        @endif
                                    @else
                                        @if ($plan->price !== ($newDiscountedPrice ?? $plan->price))
                                            <small
                                                class="mb-[0.3em] inline-flex text-[0.35em] font-normal">{{ currency()->symbol }}<span
                                                    style="text-decoration: line-through;"
                                                >{{ $plan->price }}</span></small>
                                            &nbsp;
                                            <small
                                                class="mb-[0.3em] inline-flex text-[0.35em] font-normal">{{ currency()->symbol }}</small>{{ $newDiscountedPrice }}
                                        @else
                                            <small
                                                class="mb-[0.3em] inline-flex text-[0.35em] font-normal">{{ currency()->symbol }}</small>{{ $plan->price }}
                                        @endif

                                    @endif

                                    <small class="mb-[0.3em] inline-flex text-[0.35em] font-normal">/
                                        {{ __(formatCamelCase($plan->frequency)) }}</small>
                                </div>
                                <div
                                    class="mx-auto inline-flex rounded-full bg-white p-[0.85em_1.2em] text-[15px] font-medium leading-none text-[#2D3136]">
                                    {{ __($plan->name) }}</div>
