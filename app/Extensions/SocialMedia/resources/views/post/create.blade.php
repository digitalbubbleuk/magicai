                ...
                <div @class([
                    ...
                    @foreach (\App\Extensions\SocialMedia\System\Enums\PlatformEnum::cases() as $platform)
                        ...
                        <x-button
                            ...
                            @click.prevent="currentPlatform = '{{ $platform->name }}'; platformUsername = '{{ $name }}'; platformPicture = '{!! $profileImage !!}';socialMediaPlatformId = '{{ $platform->platform()?->id }}';"
        ...
        (() => {
            document.addEventListener('alpine:init', () => {
                Alpine.data('socialMediaPostCreate', () => ({
                    ...
                    getUserPlatforms() {
                        return this.userPlatforms.filter(userPlatform => userPlatform.platform === this.currentPlatform);
                    },
                    ...
                    async postNow() {
                        ...
                        try {
                            let response = await fetch("{{ route('dashboard.user.social-media.post.store') }}", {
                                method: "POST",
                                headers: {
                            ...
                            if (result.status === 'success') {
                                ...
                                setTimeout(() => {
                                    window.location.href = '{{ route('dashboard.user.social-media.post.index') }}';
                                }, 500);
                            } else {
