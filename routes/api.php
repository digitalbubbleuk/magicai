...
// Creative Suite Templates API Routes (using session-based auth)
Route::middleware(['web', 'auth'])->prefix('creative-suite')->group(function () {
    Route::get('templates', 'App\Http\Controllers\Api\CreativeSuiteTemplateController@index');
    Route::get('templates/categories', 'App\Http\Controllers\Api\CreativeSuiteTemplateController@categories');
    Route::get('templates/popular', 'App\Http\Controllers\Api\CreativeSuiteTemplateController@popular');
    Route::get('templates/search', 'App\Http\Controllers\Api\CreativeSuiteTemplateController@search');
    Route::get('templates/{id}', 'App\Http\Controllers\Api\CreativeSuiteTemplateController@show');
});
...
Route::middleware('auth:api')->group(function () {
    ...
    Route::prefix('airealtimechat')->group(function () {

        Route::post('/stream-test', 'App\Http\Controllers\Api\AIRealTimeChatController@developerStreamTest');
        Route::post('/change-chat-title', 'App\Http\Controllers\Api\AIRealTimeChatController@changeChatTitle');
        ...
        Route::prefix('chat')->group(function () {
            Route::get('/{conver_id}', 'App\Http\Controllers\Api\AIRealTimeChatController@conversations'); // get conversation chat by conversation id
            Route::get('/{conver_id}/messages', 'App\Http\Controllers\Api\AIRealTimeChatController@conversationChats'); // get conversation chat by conversation id
            Route::get('/{conver_id}/messages/{id}', 'App\Http\Controllers\Api\AIRealTimeChatController@conversationChats'); // get conversation chat by conversation id
        });
        // must be out of chat to not to match with {conver_id}
        Route::match(['get', 'post'], '/chat-send', 'App\Http\Controllers\Api\AIRealTimeChatController@chatOutput');
        Route::post('/new-chat', 'App\Http\Controllers\Api\AIRealTimeChatController@startNewChat');

        Route::prefix('history')->group(function () { // returns all conversations of the template
            Route::get('/{cat_slug}', 'App\Http\Controllers\Api\AIRealTimeChatController@openAIChat');
            Route::delete('/', 'App\Http\Controllers\Api\AIRealTimeChatController@deleteChat');
            Route::patch('/', 'App\Http\Controllers\Api\AIRealTimeChatController@renameChat');
        });
        // must be out of history to not to match with {cat_slug}
        Route::post('/search-history', 'App\Http\Controllers\Api\AIRealTimeChatController@searchChatHistory');
        Route::get('/recent-chats', 'App\Http\Controllers\Api\AIRealTimeChatController@recentChats');
        Route::post('/search-recent-chats', 'App\Http\Controllers\Api\AIRealTimeChatController@searchRecentChats');

        Route::get('/websocket-credentials', 'App\Http\Controllers\Api\AIRealTimeChatController@getWebsocketCredentials');
        Route::post('/save-conversation', 'App\Http\Controllers\Api\AIRealTimeChatController@saveConversation');
