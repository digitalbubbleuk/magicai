...
<div
    ...
    <div
        ...
        @if (filled($post['image']))
            <img
                class="lqd-posts-item-img h-8 w-[38px] rounded-[4px] object-cover object-center shadow group-[&[data-view-mode=grid]]:mb-2 group-[&[data-view-mode=grid]]:aspect-video group-[&[data-view-mode=grid]]:h-auto group-[&[data-view-mode=grid]]:w-full group-[&[data-view-mode=grid]]:rounded-md"
    ...
    <div class="lqd-posts-item-actions flex items-center justify-end gap-2 font-normal">
        ...
        >
            ...
            <x-slot:modal>
                <form action="#" x-ref="form" x-on:submit.prevent="submitDuplicate">
                    <h3 class="mb-3.5">
                        @lang('Duplicate Post')
                    </h3>
					...
					<input type="hidden" name="route" value="{{ route('dashboard.user.social-media.post.duplicate', $post->id) }}">
                    ...
                    >
                        @foreach (\App\Extensions\SocialMedia\System\Enums\PlatformEnum::cases() as $platform)
                            <option {{ $platform->platform()?->isConnected() ? '' : 'disabled' }} value="{{ $platform->platform()?->id }}">
